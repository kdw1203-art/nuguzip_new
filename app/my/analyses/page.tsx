              ...
              return (
                <div key={r.id} className="card flex flex-col gap-1 rounded-[14px] px-4 py-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <span className="t-body font-extrabold text-ink">{title}</span>
                  ...
                  <div className="flex gap-2 pt-1">
                    <Link href={`/analysis/ai/r/${r.id}`} className="t-sub font-bold text-primary no-underline">
                      결과 스냅샷 보기 ›
                    </Link>
                    {tid && (
                      <Link href={`/analysis/ai/${tid}`} className="t-sub font-bold text-text-3 no-underline">
                        같은 도구 다시 실행 ›
                      </Link>
                    )}
                  </div>
