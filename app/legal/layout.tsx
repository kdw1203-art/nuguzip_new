    ...
    <PageShell breadcrumb="홈 › 법적 고지">
      <div className="mx-auto flex w-full max-w-5xl gap-8">
        ...
        <aside className="hidden w-52 shrink-0 lg:block">
          ...
          <nav className="space-y-0.5">
            {NAV_ITEMS.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="flex items-center justify-between rounded-[10px] px-3 py-2 text-[13px] text-text-2 hover:bg-primary-soft hover:text-ink"
              >
                {item.label}
          ...
          <div className="mt-8 rounded-[14px] bg-surface p-3 text-[12px] leading-relaxed text-text-3 border border-line">
