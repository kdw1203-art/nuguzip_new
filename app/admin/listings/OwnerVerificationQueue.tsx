...
function Row({ item }: { item: OwnerVerificationItem }) {
  ...
  return (
    <div className="flex flex-col gap-2 rounded-xl border border-[rgba(255,255,255,.08)] bg-[rgba(255,255,255,.04)] p-4">
      ...
      <div className="flex flex-wrap gap-x-3 gap-y-1 text-[12px] text-[#9aa6b8]">
      ...
      {item.note && <div className="text-[12px] text-[#c9d2e0]">신청 메모: {item.note}</div>}

      {/* 증빙 — 관리자만 접근하는 화면에서 원본을 새 탭으로 연다 */}
      <div className="flex flex-wrap gap-2">
        {item.documents.length === 0 ? (
          <span className="rounded-lg border border-[rgba(248,113,113,.35)] px-2.5 py-1 text-[10px] font-bold text-ai-danger">
            첨부된 증빙 없음
          </span>
        ) : (
          item.documents.map((d, i) => (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="rounded-lg border border-[rgba(126,162,255,.4)] px-2.5 py-1 text-[10px] font-bold text-ai-accent no-underline"
            >
      ...
      {item.status === "pending" && phase === "idle" && (
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="rounded-lg border border-[rgba(74,222,128,.4)] px-3 py-1.5 text-[12px] font-bold text-ai-success"
          >
            승인
          </button>
          <button
            ...
            className="rounded-lg border border-[rgba(248,113,113,.4)] px-3 py-1.5 text-[12px] font-bold text-ai-danger"
          >
            반려
          </button>
        </div>
      ...
      {phase === "approve" && (
        <div className="flex flex-wrap items-center gap-2">
          <span className="mr-auto text-[10px] text-[#9aa6b8]">
          ...
          <button
            ...
            className="rounded-lg border border-[rgba(255,255,255,.14)] px-3 py-1.5 text-[12px] font-bold text-[#9aa6b8]"
          >
            취소
          </button>
        </div>
      ...
      {phase === "reject" && (
        <div className="flex flex-wrap items-center gap-2">
          <input
            ...
            className="min-w-0 flex-1 rounded-lg border border-[rgba(255,255,255,.14)] bg-[rgba(255,255,255,.04)] px-2.5 py-1.5 text-[12px] text-white outline-none focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/40"
          />
          <button
            type="button"
      ...
      {phase === "busy" && <div className="text-[10px] text-[#9aa6b8]">처리 중…</div>}
    </div>
  );
}
