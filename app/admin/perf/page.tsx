...
export default async function AdminPerfPage() {
  ...
  if (!rows) {
    return <main className="p-6 text-sm text-text-2">web_vitals 조회 실패 — DB 상태를 확인하세요.</main>;
  }
  ...
  return (
    <main className="space-y-6 p-4 md:p-6">
      <header>
        <h1 className="text-lg font-bold text-ink">성능 매트릭스 — 최근 7일 p75</h1>
          ...
          <tbody>
            {paths.map((x) => (
              <tr key={x.path} className="border-b border-line/60">
                <td className="max-w-[220px] truncate px-3 py-1.5 font-mono text-[11px] text-ink">{x.path}</td>
      ...
      <section className="rounded-2xl border border-line bg-surface p-4">
        <h2 className="text-sm font-bold text-ink">LCP 범인 Top 10 (요소 선택자 기준)</h2>
        {culprits.length === 0 ? (
          ...
          <ul className="mt-2 space-y-1.5">
            {culprits.map((c) => (
              <li key={c.el} className="flex items-baseline justify-between gap-3 text-xs">
                <code className="min-w-0 flex-1 truncate text-[11px] text-text-2">{c.el}</code>
