...
/* 2026-08-06 /notes/[id] 실측: `lg:grid-cols-[1fr_400px]` 의 왼쪽 트랙이
   min-content(썸네일 10장 가로 1172px) 아래로 줄지 않아, 판단 근거·점수 축
   사이드바가 통째로 컨테이너 밖으로 나가 있었다 — 뷰포트 1296 에서 문서
   scrollWidth 1690. 저장소 전체에 같은 모양이 22곳 더 있었다.
   심은 결함으로 게이트가 실제로 잡는 것을 확인하고 넣는다(1건 → exit 1). */
runGate("UI", "본문+고정 사이드바 그리드의 minmax(0,1fr)", "check-sidebar-grid-minmax.mjs");
