...
| # | 할 일 | 어디서 | 메모 |
|---|---|---|---|
| 3 | 외부 업타임 모니터 등록: `https://naezipnow.com/api/health` 1분 간격, 알림 = 위 이메일 | UptimeRobot(무료) 등 | 내부 프로브(site-probe)는 사이트가 통째로 죽으면 같이 죽는다 |
...
| 6 | 카카오 로그인: 카카오 개발자 앱 생성 → REST 키·Client Secret → Vercel `AUTH_KAKAO_ID`·`AUTH_KAKAO_SECRET`, Redirect URI `https://naezipnow.com/api/auth/callback/kakao` | developers.kakao.com | 코드는 946 에 있고 키만 없으면 버튼이 숨는다 |
