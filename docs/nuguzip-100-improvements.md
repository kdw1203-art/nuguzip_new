...
>   손대지 않는다.**
> - **소유자 결정이 필요해 손대지 않은 것 — 판매가 이중 출처.** `app/subscription/page.tsx:19` 는
>   `lib/subscriptions/billing-periods.ts` 를 단일 출처라 선언하며 **6,900원 / 19,900원** 을
>   표시하는데, 실제로 청구하는 `app/api/payments/toss/create/route.ts:44` 는
>   `getPlan(tier).priceMonthly`(`lib/subscriptions/plans.ts:65,93`) = **2,900원 / 18,900원** 을
>   쓴다(주석: "판매가 확정 (2026-07-20 운영자 결정)"). 표시가 청구보다 **높으므로** 소비자
>   피해는 없지만, 어느 쪽이 진짜 가격인지는 **내가 정할 문제가 아니다.**
...
2. **가격 출처가 둘입니다 — 소유자 결정이 필요합니다.** `app/subscription/page.tsx:19` 는 `가격 단일 출처: lib/subscriptions/billing-periods.ts (하드코딩 금지)` 라고 선언하고 그 값을 화면에 씁니다(플러스 월 **6,900원**, 프로 월 **19,900원**). 그런데 실제로 결제창에 넘기는 금액은 `app/api/payments/toss/create/route.ts:44` 의 `getPlan(tier).priceMonthly` — `lib/subscriptions/plans.ts:65,93` 의 **2,900원 / 18,900원**(주석: "판매가 확정 (2026-07-20 운영자 결정)")입니다. **표시보다 적게 청구**되므로 소비자 피해는 아니지만, 어느 쪽이 맞는 판매가인지는 **가격 결정이라 임의로 고치지 않았습니다.** 소유자가 정한 쪽으로 다른 하나를 맞춰야 합니다.
