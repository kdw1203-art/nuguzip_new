...
export function supportFaqItems(): SupportFaqItem[] {
  ...
  return [
    /* ── 데이터·시세 ─────────────────────────────────── */
    {
      id: "data-source",
      category: "데이터·시세",
      q: "내집나우의 시세는 어디서 온 데이터인가요?",
      a: "내집나우의 시세는 전부 국토교통부 실거래가 공개시스템의 신고 자료를 공공 API(data.go.kr)로 받아 집계한 값입니다. 민간 부동산 포털이나 중개 플랫폼 화면을 긁어오지 않으며, 단지 기본정보·가격지수·청약·공매도 각각 공공 API에서 받습니다.",
      href: "/methodology",
    ...
    },
    {
      ...
      a: "아닐 가능성이 큽니다. 실거래 신고 기한이 계약일로부터 30일이라, 최근 1~2개월 수치는 아직 신고가 다 들어오지 않은 잠정치이고 앞으로 더 늘어납니다. 내집나우는 해당 기간의 리포트에 잠정 표시를 붙이고, 그 수치를 인용할 때 잠정임을 함께 밝혀 달라고 안내합니다.",
      href: "/reports",
    ...
    },
    {
      ...
      a: "거래가 없다는 뜻이 아니라 아직 수집되지 않았다는 뜻입니다. 전국 실거래 수집을 시군구 단위로 순회하며 채우는 중이라 모든 지역의 전체 기간이 채워져 있지 않고, 내집나우는 이 경우 추정치로 칸을 메우지 않고 비워 둡니다.",
    },
    {
      id: "cancelled-deal",
      category: "데이터·시세",
    ...
    },
    {
      ...
      a: "아니요. 본문 글자나 사진 속 인물·차량번호를 자동으로 감지해 가리는 기능은 아직 없습니다. 내집나우가 자동으로 처리하는 것은 사진의 EXIF 위치정보 제거뿐이므로, 공개 노트에는 동·호수처럼 특정 세대를 지목하는 정보와 타인의 얼굴·차량번호가 드러나지 않도록 직접 확인한 뒤 올려 주세요. 문제가 되는 게시물은 신고 기능으로 접수되면 검토 후 조치합니다.",
      href: "/legal/privacy",
      hrefLabel: "개인정보처리방침",
    },
    {
    ...
    },
    {
      ...
      category: "구독·결제",
      q: "결제는 어떤 방식으로 이뤄지나요?",
      a: "결제는 토스페이먼츠 등 결제대행사를 통해 처리하며, 내집나우 서버는 카드 번호 같은 결제 수단 정보를 직접 보관하지 않습니다. 결제 관련 세부 사항은 각 결제대행사의 약관을 따릅니다.",
      href: "/legal/terms#refund",
      hrefLabel: "유료 서비스 약관",
    },
    ...
    /* ── AI 분석 ─────────────────────────────────────── */
    {
      ...
      a: "내집나우 AI 에이전트는 기억이나 추정으로 시세를 말하지 않고, 국토교통부 실거래 데이터와 본인이 쓴 임장노트를 그 자리에서 직접 조회해 답합니다. 어떤 데이터를 조회했는지 목록을 답변과 함께 보여 주므로 근거를 되짚어 볼 수 있습니다. 에이전트는 본인 노트를 읽기 때문에 로그인해야 쓸 수 있습니다.",
      href: "/agent",
    ...
    },
    {
      ...
      a: "내집나우는 투자 자문을 제공하지 않습니다. AI 분석과 시장 온도 같은 지표는 공개 데이터를 요약하고 서술한 것이지 매수·매도 권유가 아니며, 실제 판단과 그 결과에 대한 책임은 이용자에게 있습니다. 대출 한도·세금처럼 제도에 따라 달라지는 항목은 반드시 금융기관·관할 관청에서 확인하세요.",
      href: "/methodology",
      hrefLabel: "지표 산출 방식",
    },
