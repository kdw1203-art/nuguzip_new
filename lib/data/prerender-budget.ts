...
/* ============================================================
   ...
   배포 #263 이 그렇게 죽었다. DB 가 밀린 그 몇 분 동안 아래 다섯 페이지가
   각자 60초를 다 태웠고, 빌드가 통째로 깨져 릴리스 자체가 나가지 못했다.

     /notes/best · /analysis/temperature · /complex/compare
   ...
   ── 상한을 20초로 잡은 이유 ────────────────────────────────────
   60초 상한의 3분의 1이다. 프리렌더 한 페이지에서 generateMetadata 와 본문이
   각각 한 번씩 읽어도(캐시가 안 먹는 최악의 경우) 40초로 상한 안에 들어온다.
   정상일 때 이 조회들은 수 초 안에 끝나므로, 20초를 넘겼다는 건 "느린 날"이
...
/** 프리렌더 한 조회에 주는 시간(ms). Next 정적 생성 상한 60초의 1/3. */
export const PRERENDER_LOAD_BUDGET_MS = 20_000;
