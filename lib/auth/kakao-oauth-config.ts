 ...
 *    - `https://naezipnow.com/api/auth/callback/kakao`
 *    - `https://www.naezipnow.com/api/auth/callback/kakao` (www 사용 시)
 *    - `http://localhost:3000/api/auth/callback/kakao` (로컬)
 * 3. 동의항목: 닉네임(필수), **카카오계정 이메일(선택 동의 권장)**
 *    — 이메일 동의항목은 비즈 앱 전환 후 사용 가능. 이메일 미제공 사용자는
 *      `kakao-{id}@noreply.naezipnow.com` 합성 주소로 세션 식별만 한다(발송 불가 주소).
 * 4. 앱 키 → Vercel 환경변수:
