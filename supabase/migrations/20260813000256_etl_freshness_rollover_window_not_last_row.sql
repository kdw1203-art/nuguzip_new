-- 2026-08-13: month_rollover 거짓 critical 수정 — 마지막 로그 1행 대신 최근 26시간 최대 대상월.
-- 실측: 08-12 10:03 ETL 이 202608 수집(10:03:48) 직후 202607 전월 보강(10:03:49)을 돌려
-- 마지막 로그가 전월이 됐고, 당월을 수집하고도 critical 이 표시됐다. 새 etl.yml 은
-- 매일 "당월 → 전월 보강" 순서로 돌므로 이 거짓 경보는 매일 재발한다 — 가끔 틀리는
-- 게이트는 아무도 안 보는 게이트다. 변경은 molit_target CTE 한 곳뿐, 나머지는
-- 20260811223945 본문 그대로다.
-- 롤백: 20260811223945_etl_freshness_http_probe_check_and_honest_rum.sql 본문 재적용.
...
molit_target as (
  ...
  -- ETL 이 당월 수집 뒤 전월 보강을 돌리면 마지막 로그가 항상 전월이라,
  -- 당월을 수집하고도 critical 이 났다(실측: 08-12 10:03 — 202608 수집 1초 뒤
  -- 202607 보강 로그가 덮음). 26시간 내 로그가 없으면 최신 1행으로 폴백해
  -- "롤오버 안 됨" 판정 자체는 유지한다.
  select coalesce(
